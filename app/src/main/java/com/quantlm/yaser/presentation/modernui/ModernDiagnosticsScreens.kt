...
fun ModernSystemLogsScreen(
    ...
    LaunchedEffect(autoRefreshEnabled) {
        if (!autoRefreshEnabled) return@LaunchedEffect
        while (isActive) {
            refreshLogsOnce()
    ...
    Scaffold(
            ...
            TopAppBar(
                title = {
                    Row(
                        ...
                        IconButton(
                            onClick = { scope.launch { refreshLogsOnce() } },
        ...
        if (!ready) {
            ...
            val logsText = remember(lines) { lines.joinToString("\n") }
            Column(
                modifier = Modifier
                    .fillMaxSize()
                    .padding(padding)
                ...
                Text(
                    text = "Tip: long-press any log text to select and copy.",
                    style = MaterialTheme.typography.labelSmall,
                ...
                Row(
                    ...
                    FilterChip(
                        ...
                        label = { Text(if (autoRefreshEnabled) "Auto refresh: On" else "Auto refresh: Off") }
                    )
                    FilterChip(
                        selected = followTailEnabled,
                        onClick = { followTailEnabled = !followTailEnabled },
                        label = { Text(if (followTailEnabled) "Follow tail: On" else "Follow tail: Off") }
                    )
                    OutlinedButton(
                        onClick = { scope.launch { contentScrollState.animateScrollTo(0) } }
                    ) {
                        Text("Top")
                    }
                    OutlinedButton(
                        onClick = { scope.launch { contentScrollState.animateScrollTo(contentScrollState.maxValue) } }
                    ) {
                        Text("Bottom")
                    }
                }
